...
File.open("_data/#{ENV['AIRTABLE_TABLE']}.yml", 'w') do |file|
  ...
  data.each{|e|
    ...
    e[:permalink]="eventos/#{e[:comienzo].to_date.to_s}-#{e[:name].downcase.gsub(/[^a-z0-9áéíóúÁÉÍÓÚ]/,"-").split.compact.join("-")}-#{e[:id].downcase.split.join("-")}"
  }
  expanded = []
  data.each do |e|
  ...
  FileUtils.mkdir_p("_events")
  data.each{|p|
    File.write(
      "_events/#{p[:id].downcase.split.join("-")}.md",
      "---
layout: post
permalink: #{p[:permalink]}
title: #{p[:name].gsub(":", "")} - #{p[:comienzo].to_date}
---
#{embedig(p[:url])}
"
    )
